...
const Checkout = () => {
  ...
  const { name, img, details, videos, price } = course;
  return (
    <div className="bg-secondary p-10 rounded-xl w-4/5 mx-auto">
      ...
      <p className="mt-8 text-xl md:text-2xl">Total Videos: {videos}</p>
      <p className="mt-8 text-xl md:text-xl">Price: {price}</p>
      <p className="mt-8 text-xl md:text-2xl">{details}</p>
